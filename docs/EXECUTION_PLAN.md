...
| 2026-09-13 | P2.3 (CI) | **اختبار توقيت متقلقل**: `memory/performance-benchmark` كان يؤكد عتبة زمن جداري (20ms) على **عيّنة واحدة** أثناء تشغيل كل الحزم على التوازي، فتوقف GC واحد أو استباق جدولة يكفي لإسقاطه. أُضيف `bestOf(N)` يأخذ أدنى العيّنات بعد تسخين — مستقر تحت التزاحم وما يزال يمسك الانحدار الحقيقي لأن البطء الخوارزمي يُبطئ كل عيّنة | كان 2334 ناجح/1 فاشل؛ صار **2335/0 في تشغيلتين متتاليتين** للمجموعة الكاملة |
